...
const weaponRules = {
  // M04 指对数模块专用杀手锏
  'M04': {
    priority: ['S-FUNC-02', 'S-FUNC-04', 'S-FUNC-05', 'S-FUNC-06', 'S-FUNC-08', 'S-DERIV-03', 'S-DERIV-04', 'S-DERIV-09', 'S-DERIV-10', 'S-DERIV-11', 'S-INEQ-02', 'S-INEQ-05', 'S-INEQ-06', 'S-INEQ-07', 'S-INEQ-10', 'S-SEQ-02', 'S-SEQ-04', 'S-SEQ-08', 'S-SEQ-09', 'S-SEQ-10'],
    keywords: {
  ...
  // M05 向量模块专用杀手锏
  'M05': {
    priority: ['S-VEC-01', 'S-VEC-02', 'S-VEC-03', 'S-VEC-04', 'S-VEC-05'],
    keywords: {
...
function matchWeaponsPrecise(problem, answer, keyPoints, analysis, level, varName, motifId) {
  ...
  // 获取该模块的优先杀手锏
  const moduleRules = weaponRules[motifId] || { priority: [], keywords: {} };
  const commonKeywords = weaponRules.common.keywords;
  
  // 合并关键词
  const allKeywords = { ...commonKeywords, ...(moduleRules.keywords || {}) };
  
  // 计算匹配分数
  for (const [weaponId, keywords] of Object.entries(allKeywords)) {
    ...
    if (score >= 2) {
      // 模块优先杀手锏加分
      if (moduleRules.priority && moduleRules.priority.includes(weaponId)) {
        score += 2;
      }
