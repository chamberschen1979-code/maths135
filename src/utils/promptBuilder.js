...
>>> **若为 A类 [几何/向量]：**
1. **动态性/定值 判别测试**：
   - 选取动点的**两个极端位置**（如圆的最左/最右端，或线段端点），分别计算目标量 Val1, Val2。
   - **关键判断**：
     - 若题目问"**范围/最值**"：必须满足 **Val1 != Val2**。若相等，说明是定值，**立即报错**（严禁定值问范围）。
     - 若题目问"**求值**"：必须满足 **Val1 == Val2**。若不相等，说明条件不足或构型错误，**立即报错**。
...
2. **参数解的形态检查 (L3 核心)**：
   - **若题目问"求值"**：解必须唯一。若解不唯一（如 ω = 6k-1），题干**必须**有限制条件（如"最小正值"、"ω∈(0,10)"）。若无限制，**立即报错**。
   - **若题目问"取值范围"**：**允许且鼓励**解为一个区间（无穷多解）。此时**严禁**报错"解不唯一"。只需验证该区间是否连续、边界是否合理。
...
**第四步：数值合理性自查 (最高优先级)**
在输出最终答案前，**必须**执行以下反向验算：
1. **代入验算**：将答案代入原方程/原条件进行反向验算。
   - 若验算不成立（如 1 ≠ -1），**严禁强行输出答案**，必须重新生成题目或直接报错重试。
2. **唯一性检查**：检查条件是否不足导致结果不唯一（如向量题中基底退化）。
   - 若条件不足且题目问"求值"，**严禁强行输出单一答案**，必须补充条件或改问"求范围"。
3. **边界验证**：若答案为范围，验证边界值是否确实满足所有条件。

**第五步：最终自检填空 (基于上述真实计算如实填写)**
- 归属类别：(填写 A/B/C/D/E)
- 核心验算结果：(填写具体算式，如 'Val1=-1, Val2=3, 不相等 -> 符合范围题' 或 'Sum(P)=1' 或 'n=1 符合')
- 数值反向验算：(填写 '代入验证通过' 或 '发现矛盾: [具体问题]')
...
1. **自然格式**：可以直接输出 JSON 字符串，也可以使用 markdown 代码块包裹
   - 示例 1（直接输出）：{"question": "..."}
   - 示例 2（代码块）：使用 \`\`\`json ... \`\`\` 包裹
   - 解析器已内置容错处理，两者都会被正确解析
...
4. **🔴 L4 难度特别提示**：
   - 如果题目包含复杂的分段函数或分类讨论，请将数学推导过程放在 'analysis' 字段
   - 'answer' 字段仅保留最终结论的简洁 LaTeX 表示
   - 避免在 JSON 值中包含未转义的换行符或双引号
   - 如果答案需要分情况讨论，请使用数组格式：\\"answer\\": [\\"情况1: ...\\", \\"情况2: ...\\"]
   - 确保 JSON 字符串值内部的双引号已转义：\\\\\\"\\\\\\"

## 🚨 自检指令 (核心)
在生成题目后，请在内心进行一次自我审查：
1. 题目是否存在"定值问范围"的逻辑谬误？
2. 所有参数是否已明确定义？
3. 答案是否唯一且合理？

若发现任何逻辑漏洞，请立即在内部修正，不要输出有缺陷的题目。
...
**验证流程**：逆向构造 → 逐条验证不变量 → 通过则继续，不通过则重造。`
  ...
  // 🔥 新增：动态防御指令 (基于题型特征，而非母题 ID)
  const dynamicConstraints = []
  
  // 获取上下文字符串用于特征检测
  const contextText = `${motifName} ${specName} ${varName} ${benchmarkText} ${seedText || ''}`
  
  // 🔥 全局规则 1: 防止"定值问范围" (适用于所有几何/向量/函数题)
  if (varName.includes("范围") || varName.includes("最值") || 
      /范围|最值|取值范围/.test(contextText)) {
    dynamicConstraints.push(
      `🛑 **逻辑自洽铁律 (Anti-Degeneracy)**:
      既然题目要求求"范围"或"最值"，你必须确保目标量随变量变化！
      - **动作**: 在草稿中选取变量的**两个极端值**（如起点、终点、对称点）进行试算。
      - **红线**: 如果两个值相等，说明是**定值**！严禁问"范围"。
      - **修正**: 若发现是定值，必须修改几何构型（如移动圆心、改变角度）或更改变式方向（改为求值）。`
    )
  }
  
  // 🔥 全局规则 2: 防止"题干即答案" (适用于所有代数/逻辑题)
  if (/求.*值|求证|计算/.test(contextText)) {
    dynamicConstraints.push(
      `🛑 **非废话铁律 (No-Tautology)**:
      - **自检**: 题干中的已知条件是否直接包含了问题的答案？(例如：已知 a+b=1，求 a+b)
      - **红线**: 严禁出现这种逻辑循环。学生必须经过至少一步推导才能得出答案。
      - **验证**: 在 reasoning 中显式写出"学生推导路径"，确认至少包含一个非平凡步骤。`
    )
  }
  
  // 🔥 全局规则 3: 高一工具限制 (适用于所有函数题)
  if (userGrade === '高一' && /ln|e\^|导数|切线|单调性|极值/.test(contextText)) {
    dynamicConstraints.push(
      `🛑 **高一工具红线**:
      严禁使用导数工具分析 $x\\ln x$, $xe^x$ 等超越函数的单调性。
      - **替代**: 若必须考查此类函数，请改为"验证解的区间"（代入法）或更换为初等函数（二次/对勾/指数）。
      - **禁止**: 严禁在解析中出现 $f'(x)$、求导、导数等术语。`
    )
  }
  
  // 🔥 全局规则 4: 概率归一性检查 (适用于所有概率题)
  if (/概率|分布|期望|方差|P\(/.test(contextText)) {
    dynamicConstraints.push(
      `🛑 **概率归一铁律**:
      - **验证**: 所有概率值之和必须严格等于 1。
      - **动作**: 在 reasoning 的 step3_adaptive_check 中，显式计算 $\\sum P_i$ 并确认等于 1。
      - **红线**: 若和不等于 1，必须重新分配概率值。`
    )
  }
  
  // 🔥 全局规则 5: 三角函数相位唯一性 (适用于所有三角题)
  if (/三角|sin|cos|tan|相位|周期/.test(contextText) && varName.includes("求")) {
    dynamicConstraints.push(
      `🛑 **三角相位唯一性铁律**:
      - **验证**: 若题目要求求 φ 或 ω，必须检查是否存在多解。
      - **动作**: 题干中必须给出足够的约束条件（如相位范围、区间限制）确保解的唯一性。
      - **红线**: 若无约束导致无穷多解，必须补充条件或更换问法。`
    )
  }
  
  // 构建动态防御段落
  let dynamicConstraintsSection = ''
  if (dynamicConstraints.length > 0) {
    dynamicConstraintsSection = `
## 🔥 动态防御铁律 (基于题型特征自动触发)
以下规则根据题目特征自动激活，违反任意一条即视为命题失败：
${dynamicConstraints.map((c, i) => `${i + 1}. ${c}`).join('\n\n')}

**执行要求**: 在 reasoning 的 step3_adaptive_check 中，必须逐条验证上述规则并记录结果。`
  }
...
` : ''}
${strategyComparisonSection}${strategySection}${levelConstraintsSection}${hardConstraintsSection}${moduleConstraintsSection}${mathInvariantsSection}${dynamicConstraintsSection}${weaponSection}
## 生成指令
...
⚠️ **后端独立复核警告**：你的验算过程将被后端独立复核！
- 如果发现验算步骤与题目结论不符，或验算过程造假，题目将被直接驳回。
- 后端会使用独立的 LLM 和硬规则守卫进行验证，不要抱有侥幸心理。
- 请确保你的 reasoning 中的验算是真实、可追溯的数学推导。

请在 'reasoning' 的 step3_adaptive_check 最后，**必须以填空题的形式**，逐一如实回答以下五个问题：

1. **反平庸与基因继承检查**：你是否提取了种子题的独特数学结构（如非线性、绝对值、偏心几何等），而没有退化为随处可见的基础模板题？(填写：是/否/无种子题)
2. **自适应数学严谨性检查**：你是否已经在草稿中执行了特定领域的验算？
   - 若为**范围题**：是否确认了目标量确实**随变量变化**（非定值）？
   - 若为**多解题**：是否确认了题干已给出足够约束（若求唯一值）或 允许解集为区间（若求范围）？
   - (填写：是/否/非本题)
   *注意：L3 题目允许解为区间或分类讨论的结果，这不属于"不严谨"。*
...
5. **🚨 逻辑冗余与废话检测 (Critical)**：
   - **自问**："学生是否需要经过推导才能得出答案？"
   - **严禁**：出现"已知 A+B=1，求 A+B"这类题干直接包含答案的情况。
   - **严禁**：出现"已知条件 X 是错误的，请指出"这类非求解型题目（除非明确要求辨析）。
   - **严禁**：题干中出现与求解无关的冗余条件，或条件之间相互包含。
   - **动作**：如果发现此类情况，**必须立即重写**，将问题改为求解具体的变量值或范围。
   - (填写：无冗余/发现冗余已修正)

**🔥 组长最高警告**：如果你对任何一个问题回答了"否"、"冲突"或"发现冗余未修正"，说明产生了致命数学幻觉！**必须立刻清空当前参数，重新提取基因构造！** 只有全票通过，才允许生成最终的 question 和 JSON！`
}
